...
def rec_make_pair(coords: Tuple[int, int, int], augment: int,
    ...
    if augment == 0:
        ...
        x0, next0 = rec_make_pair((2*x, 2*y, z+1), augment-1, ag, target, buffer)
        x1, next1 = rec_make_pair((2*x+1, 2*y, z+1), augment-1, ag, target, buffer)
        x2, next2 = rec_make_pair((2*x, 2*y+1, z+1), augment-1, ag, target, buffer)
        x3, next3 = rec_make_pair((2*x+1, 2*y+1, z+1), augment-1, ag, target, buffer)

        img = np.vstack([np.hstack([x0, x1]), np.hstack([x2, x3])])
        next_ = np.hstack([next0[0], next3[0][1:]]), np.hstack([next3[1], next0[1][1:]])
...
def bounding_box(name: str, tag: Dict[int, str],
